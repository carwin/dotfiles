...
The =cy-systems= package requires a couple of Org & babel functions, and
nearly everything else depends on that package for per-system
configuration. To handle this, =cy-systems= needs to be loaded quite
early in the init process and must come after Org. This is important
because if not done this way, the built-in version of Org will be
evaluated rather than the latest, which causes all sorts of issues.
...
With Org loaded, the per-system settings in =cy-system= can be loaded:
...
Rebind =S-<return>= to make it easier to continue lists or headings
while in =evil='s insert mode because forwarded X sessions of Emacs
can't always capture the =M-= key bindings properly in all cases.
...
(if (eq system-type 'darwin)
    (use-package alfred-org-capture
      :straight (
                 :type git
                 :host github
                 :repo "carwin/alfred-org-capture"
                 :files (:defaults "el/*"))))
#+end_src
...
typing a ~<~ followed by the template name (e.g.: ~el~, ~py~) and pressing
~TAB~.
...
It's nice to have a table of contents section for long literate config
files (like this one) and for really long documents and long-running
notes about various topics that only continue to grow. =org-make-toc=
can do this.
...
#+begin_src emacs-lisp
(defvar cy/org-roam-project-template
...
Function to filter roam content by tag

#+begin_src emacs-lisp
(defun cy/org-roam-filter-by-tag (tag-name)
  (lambda (node)
    (member tag-name (org-roam-node-tags node))))
#+end_src

Function to list notes by tag

#+begin_src emacs-lisp
(defun cy/org-roam-list-notes-by-tag (tag-name)
  (mapcar #'org-roam-node-file
          (seq-filter
           (cy/org-roam-filter-by-tag tag-name)
           (org-roam-node-list))))
#+end_src

Enable immediate roam capture

#+begin_src emacs-lisp
(defun cy/org-roam-node-insert-immediate (arg &rest args)
  (interactive "P")
...
Refresh org roam agenda list

#+begin_src emacs-lisp
(defun cy/org-roam-refresh-agenda-list ()
  (interactive)
  (setq org-agenda-files (cy/org-roam-list-notes-by-tag "Project")))
#+end_src
