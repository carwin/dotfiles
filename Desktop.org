...
export PATH="/usr/local/bin/gnu-getopt/bin:$PATH"
#+end_src
...
=fnm= replaces the usage of =nvm= found in older versions of this configuration.
...
Zsh is the preferred shell for this setup. Bash is fine, but doesn't
have some of the completion features I like from Zsh. Fish is great,
and handy for out-of-the-box configuration and features, but has some
annoying compatibility issues (perhaps quirks is a better word) that
make running Bash on the fly rather annoying. These quirks are easily
overcome, but my personal opinion is that I shouldn't have to.

To keep things simple and as bloat-free as possible, while still
having nice prompt info and other QOL benefits, this configuration
uses the =slimzsh= configuration.

**** =aliases.zsh.local=

Create a file just for handy user-defined aliases. This file, if
tangled into the =slimzsh= configuration's directory, gets automatically
sourced. For other configurations, adjust the tangle directive of the
last block in this section to put it somewhere else and source the
file manually.
...
#+begin_src conf :noweb-ref zsh-aliases :noweb yes
...
alias python='/usr/local/bin/pyenv exec'
alias pip='/usr/local/bin/pip3'
#+end_src

#+begin_src conf :noweb yes :tangle (concat (cy/system-settings-get 'user/config) "zsh/slimzsh/aliases.zsh.local")
<<zsh-aliases>>
#+end_src
...
# Evaluate the fnm Node.js version manager environment vars.
eval "$(fnm env)"
#+end_src
...
Typically, this is where the majority of Zsh customization takes
place, however, because we're opting to use the =slimzsh= configuration,
all that needs to be done is to source its entry point.
