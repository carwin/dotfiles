...
#+begin_src shell :noweb-ref pacman-install
...
#+begin_src shell :noweb-ref arch-install-yay
cd ~/;
...
#+begin_src shell :noweb-ref yay-install
yay -S \
    ack \
    aaxtomp3 \
...
#+begin_src shell :noweb-ref get-preferred-fonts
...
#+begin_src conf :noweb yes :tangle (concat (cy/system-settings-get 'user/home) "/.profile") :mkdirp yes
<<dot-profile>>
#+end_src
...
#+begin_src conf :tangle (concat (cy/system-settings-get 'config/dir) "/user-dirs.dirs") :mkdirp
...
#+begin_src conf :tangle (concat (cy/system-settings-get 'config/dir) "/zsh/.zprofile") :mkdirp yes
...
#+begin_src conf :tangle (concat (cy/system-settings-get 'config/dir) "/zsh/.zshrc") :mkdirp yes :noweb yes
<<zshrc>>
#+end_src
...
#+begin_src conf :noweb yes :tangle ~/.Xresources :mkdirp yes
<<xresources-config>>
#+end_src
...
#+begin_src shell :shebang #!/bin/bash :noweb-ref source-xresources
xrdb ~/.Xresources
#+end_src
...
#+begin_src shell :noweb yes :tangle ~/.config/i3/bin/autostart.sh :mkdirp yes :shebang #!/bin/bash :tangle-mode (identity #o755)
<<i3-autostart-script>>
#+end_src
...
#+begin_src shell :noweb-ref i3-autostart-script
...
#+begin_src shell :noweb yes :tangle (concat (cy/system-settings-get 'config/dir) "/i3/bin/i3lock.sh") :mkdirp yes :tangle-mode (identity #o755) :shebang  #!/bin/bash
<<i3-lock-script>>
#+end_src

Define the color variales the script will use:

#+begin_src shell :noweb-ref i3-lock-script
FG="#c8ccd4"
BG="#1e222a"
...
#+begin_src shell :tangle (concat (cy/system-settings-get 'config/dir) "/i3/bin/launch-dunst.sh") :mkdirp yes :tangle-mode (identity #o755) :shebang #!/bin/bash
if [[ `pidof dunst` ]]; then
    pkill dunst
fi
...
#+begin_src shell :noweb yes :tangle (concat (cy/system-settings-get 'config/dir) "/i3/bin/launch-polybar.sh") :mkdirp yes :tangle-mode (identity #o755) :shebang #!/bin/bash
<<i3-polybar-launcher>>
#+end_src

Terminate already running bar instances.

#+begin_src shell :noweb-ref i3-polybar-launcher
killall -q polybar
#+end_src
...
#+begin_src shell :noweb yes :tangle (concat (cy/system-settings-get 'config/dir) "/i3/bin/launch-compton.sh") :mkdirp yes :tangle-mode (identity #o755) :shebang #!/bin/bash
<<i3-compositor-launcher>>
#+end_src

Terminate the compositor process if compton/picom is already running.

#+begin_src shell :noweb-ref i3-compositor-launcher
killall -q compton
#+end_src
...
#+begin_src conf :noweb yes :tangle ~/.config/dunst/dunstrc :mkdirp yes
<<dunst-config>>
#+end_src
...
#+begin_src python :tangle ~/.config/rofi/bin/network-menu :mkdirp yes :tangle-mode (identity #o755)
#!/usr/bin/env python3
# encoding:utf8
...
#+begin_src conf :noweb yes :tangle ~/.config/mpv/mpv.conf :mkdirp yes
<<mpv-config>>
#+end_src
...
#+begin_src conf :noweb yes :tangle ~/.config/mpd/mpd.conf :mkdirp yes :comments org
<<mpd-config>>
#+end_src
...
#+begin_src emacs-lisp :noweb yes :tangle ~/.config/ncmpcpp/config :mkdirp yes
<<ncmpcpp-config>>
#+end_src
...
#+begin_src shell :noweb yes :shebang #!/bin/bash :output both :results verbatim
<<pacman-install>>
<<arch-install-yay>>
<<yay-install>>
<<get-preferred-fonts>>
